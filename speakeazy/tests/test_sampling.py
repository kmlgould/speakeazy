...
class TestSampler():
    def setup(self):
         ...
         _spec_file = os.path.join(spec_dir, 
                                          'bb1.spec.fits')
         spectrum = data.Data(spectrum_file=_spec_file,photometry_file=None,run_ID='/test_samp',phot_id=None)
         prs = priors.Priors(spectrum)
         
         prs.params['broadlines']=False
         
         fit_object = fitting.Fitter(spectrum,prs)
         fit_object.fit_redshift_chisq()
         fit_object.measure_velocity_prior(xlims=0.15)
